        ...
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex bg-gray-200/50 p-1.5 rounded-2xl">
                    <button onClick={() => setActiveTab('territories')} className={`px-6 py-2 text-sm font-bold rounded-xl transition-all ${activeTab === 'territories' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}>Mapas</button>
                    <button onClick={() => setActiveTab('users')} className={`px-6 py-2 text-sm font-bold rounded-xl transition-all ${activeTab === 'users' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}>Usuários</button>
                </div>
            </div>
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        ...
                                                        {availableMapsOptions.map(m => (
                                                            <option key={m.id} value={m.id}>{m.name} {isRecentWork(m.history) ? '(Descanso)' : ''}</option>
                                                        ))}
                                                    </select>
